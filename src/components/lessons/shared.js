...
// Рендира блоковете съдържание (текст, заглавие, списък, цитат, код, очакван резултат).
export function Blocks({ blocks, lang }) {
  ...
  return (
    <div className="flex flex-col gap-4">
      {blocks.map((b, i) => {
        ...
        // кодов къс в текста (например: точният текст, който трябва да се напише)
        if (b.type === 'code') return (
          <pre key={i} className="rounded-lg border border-white/10 bg-black/40 px-4 py-3 text-[13px] leading-relaxed overflow-x-auto">
            <code className="text-sky-200 whitespace-pre-wrap">{b.code ?? b[lang]}</code>
          </pre>
        );
