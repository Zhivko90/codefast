...
---
- **54-semicolon** — липсващата точка и запетая изяжда и декларацията под себе си. Парсерът хвърля недовършената, търси напред до първата точка и запетая и продължава оттам. Умират две, не всичко до края на блока. Без съобщение за грешка.
...
### ⚠ СЛАБИ ПРОВЕРКИ — хванати на място

**pattern: "." е нула проверка.** Съвпада с всеки непразен низ. А computed
стойността почти никога не е празна — браузърът връща своя подразбиращ се
шрифт, размер, тегло. Проверката минава винаги, дори при изтрит CSS.
Хванато в 07-first-code: „Задал си шрифт" минаваше, докато styles.css
изобщо не стигаше до страницата.

**Решението: style_is_not с expect: "initial".**
`initial` е изричната начална стойност на свойството, без наследяване.
Пробата я смята на машината на ученика и сравнява срещу нея.
Работи за font-family, font-size, color — за всяко свойство, където
питаш „пипал ли си това изобщо".

**Правилото:** преди да напишеш style_matches, питай се дали образецът
може да падне. Не може ли — това не е проверка, а украса.
...
---
## Доклад за грешка — ГОТОВ, но БЕЗ ИЗВЕСТИЯ

Бутон с триъгълник, най-долу във вертикалната лента (десктоп) и накрая
на таб лентата (телефон). Отделен от работните бутони — кошчето е горе,
до резултата.

Пише в таблица `feedback` в Supabase.

⚠ КОДЪТ ЗАМИНАВА С ДОКЛАДА. „Не работи" не се поправя. Ученикът вижда
отметката и може да го изключи. При няколко файла се пращат ВСИЧКИТЕ,
с имена — иначе не се разбира кой е бил счупен.

⚠ БЕЗ ПРОФИЛ СЪЩО МОЖЕ. Иска ли се профил, губиш точно докладите от
хората, които още решават дали да останат.

⚠ ТАБЛИЦАТА НЯМА ПОЛИТИКА ЗА ЧЕТЕНЕ. Само insert. Значи никой не може
да дърпа чужди доклади (и чужд код) през API-то.

### ⚠ ДУПКА: няма известия

Разбираш за доклад САМО ако влезеш в Supabase и погледнеш:

  select created_at, course, item_id, kind, message, code, lang, url
  from feedback where status = 'new' order by created_at desc;

  update feedback set status = 'done' where id = '...';

Три начина да се запуши, по цена:
- отметка в календара — нула работа, зависи от паметта
- Database Webhook → Resend, имейл при нов ред — половин час, безплатно
- страница /admin/feedback със списък и бутон „готово" — струва си, когато
  докладите станат много

Отложено нарочно. Ако почнат да се губят доклади — това е причината.

## Преводите

Всички низове в WorkBench минават през next-intl, ключове в
src/messages/{bg,en}/practice.json, ПЛОСКИ.

⚠ Бутоните на наставника бяха зашити на български месеци наред и никой
не забеляза, защото английският фолбек мълчи. Като добавяш бутон в
WorkBench — веднага ключ, не „после".

## Разцепването на WorkBench

Беше 800 реда. Стана:
  workbench/ResultPanel.js    — проверки, „защо не мина", стълбата, наставникът
  workbench/PreviewPane.js    — прегледът
  workbench/FeedbackDialog.js — докладът
  workbench/Buttons.js        — бутоните на двете ленти + иконите
  workbench/guard.js          — обезвредява връзките в превюто
  WorkBench.js                — само подредбата, под 400 реда

Иконите са на едно място, защото всяка се ползва два пъти — веднъж на
десктоп, веднъж на телефон. Разминат ли се, лентите изглеждат различно.
